...
/**
 * Generates a normal map from a canvas by treating luminance as height.
 * Uses Sobel filter to compute surface gradients.
 */
function generateNormalMap(canvas, strength = 1.5, repeat = false) {
  ...
  return canvasToTexture(normalCanvas, repeat);
}

function createTextureSet(canvas, repeat = false, normalStrength = 1.5) {
  return {
    map: canvasToTexture(canvas, repeat),
    normalMap: generateNormalMap(canvas, normalStrength, repeat),
  };
}
...
export function createDirtTexture(size = 512) {
  ...
  // Fine grain noise for micro detail
  for (let y = 0; y < size; y += 2) {
    for (let x = 0; x < size; x += 2) {
      const n = smoothNoise(x * 0.15, y * 0.15);
      ctx.fillStyle = `rgba(${n > 0.5 ? 55 : 25},${n > 0.5 ? 50 : 22},${n > 0.5 ? 40 : 18},0.08)`;
      ctx.fillRect(x, y, 2, 2);
    }
  }

  return createTextureSet(canvas, true, 2.0);
}
...
export function createPathTexture(size = 256) {
  ...
  return createTextureSet(canvas, true, 1.5);
}

export function createBarkTexture(w = 256, h = 512) {
  const canvas = document.createElement('canvas');
  canvas.width = w;
  canvas.height = h;
  const ctx = canvas.getContext('2d');
  ...
  return createTextureSet(canvas, true, 2.5);
}

export function createFoliageTexture(size = 256) {
  const canvas = document.createElement('canvas');
  canvas.width = canvas.height = size;
  const ctx = canvas.getContext('2d');
  ...
  return createTextureSet(canvas, true, 0.5);
}
...
export function createDeadWoodTexture(size = 256) {
  const canvas = document.createElement('canvas');
  canvas.width = canvas.height = size;
  const ctx = canvas.getContext('2d');
...
export function createFenceTexture(w = 64, h = 256) {
  const canvas = document.createElement('canvas');
  canvas.width = w;
  canvas.height = h;
  const ctx = canvas.getContext('2d');
...
export function createGravestoneTexture(size = 256) {
  const canvas = document.createElement('canvas');
  canvas.width = canvas.height = size;
  const ctx = canvas.getContext('2d');
  ...
  return createTextureSet(canvas, true, 1.8);
}

export function createIronTexture(size = 128) {
  const canvas = document.createElement('canvas');
  canvas.width = canvas.height = size;
  const ctx = canvas.getContext('2d');
  ...
  return createTextureSet(canvas, true, 1.2);
}
